...
class RecursiveKeyWordSetter:
	def __init__(self):
		self.class_config_map = {} #{widget_class: {key: value}}
...
'''
RecursiveBindSetter is used to recursively bind events to functions for full widget child-trees.
Keeps an interal dictionary mapping tkinter widget classes to dictionaries events and functions.
Functions passed to the binding map must take a tkinter event as the first positional argument. 
...
class RecursiveBindSetter:
	def __init__(self):
		self.class_bind_map = {} #{widget_class: {event: function}}

	def set_bind(self, widget_class, event, func):
		self.class_bind_map.setdefault(widget_class, {})[event] = func

	def clear_binds(self):
		self.class_bind_map = {}

	def apply_binds(self, widget):
		for widget_class, event_dict in self.class_bind_map.items(): #for all widget_class and binding specified in the map,
			if isinstance(widget, widget_class): #check if widget is an instance of a widget_class
				for event, func in event_dict.items(): 
					widget.bind(event, lambda event, f=func: f(event), add='+') #bind each event to each func for that widget

		for child in widget.winfo_children():
			self.apply_binds(child) #recurse through all child widgets