...
class Model:
	...
	def propagate(self):
		particle_orbit = self.particle.orbit.copy()

		for _ in range(self.lattice.num_cells):
			for _ in range(self.lattice.quad_divisions):
				next_coordinates = self.lattice.quad_s_vector + self.lattice.fquad_matrix @ particle_orbit[-1]
				particle_orbit.append(next_coordinates)

			for _ in range(self.lattice.drift_divisions):
				next_coordinates = self.lattice.drift_s_vector + self.lattice.drift_matrix @ particle_orbit[-1]
				particle_orbit.append(next_coordinates)

			for _ in range(self.lattice.quad_divisions):
				next_coordinates = self.lattice.quad_s_vector + self.lattice.dquad_matrix @ particle_orbit[-1]
				particle_orbit.append(next_coordinates)
		...
		self.particle.set_orbit(particle_orbit)
