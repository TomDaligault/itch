...
class Presenter:
	def __init__(self, ui, model):
		...
		self.cell_divisions = {'drift_divisions': 9, 'quad_divisions': 9}
	...
	def randomize_particle(self):
		self.ui.set_particle_inputs(round(np.random.normal(),2), round(np.random.normal(),2))
...
#Restores default, then set exercises based on the name of the tab that was clicked.
	...
	def update_plot_markers(self):
		length = self.model.get_num_lattice_elements()
		self.ui.update_cell_scale(scale_length=length)
		scale_value = self.ui.get_cell_scale_value()
		self.ui.plots_widget.set_markers(marker_start=scale_value, marker_spacing=length)
